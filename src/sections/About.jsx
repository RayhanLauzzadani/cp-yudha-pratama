...
export default function About() {
  ...
  const runAnimation = () => {
    ...
    // Mobile: offset lebih kecil agar tidak terlalu ke kiri
    const OFFSET_X_INITIAL = isMobile ? -185 : -350;
    ...
    // Mobile: scale lebih besar agar lebih terlihat
    elPink.style.setProperty("--sEnd1", isMobile ? "0.12" : "0.07");
    elPink.style.setProperty("--sEnd2", isMobile ? "0.15" : "0.09");
    ...
    const onStage1End = () => {
      ...
      const onPopEnd = () => {
        ...
        setTimeout(() => {
          // Mobile: final position lebih ke kanan
          const OFFSET_X_FINAL = isMobile ? -20 : -30;
  ...
  useEffect(() => {
    return () => {
      movingPinkRef.current && (movingPinkRef.current.style.animation = "none");
      grayRef.current && (grayRef.current.style.animation = "none");
    };
  }, []);

  return (
    <section ref={sectionRef} id="about" className="relative w-full bg-white py-20 font-jakarta">
      <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row items-center gap-10 px-6 sm:px-10">
        {/* FOTO + DEKOR - Mobile: jarak 40px dari kanan */}
        <div className="flex-shrink-0 flex lg:justify-center justify-end w-full lg:w-1/2 lg:pr-0 -mt-20">
          <div
            role="button"
            tabIndex={0}
            onClick={runAnimation}
            onKeyDown={onKey}
            className="relative w-[200px] sm:w-[420px] md:w-[480px] lg:w-[520px] xl:w-[560px] outline-none"
        ...
        <div className="text-[#383737] lg:w-1/2 text-left relative z-40 lg:mt-0 -mt-10">
          ...
          <h2 
            className="text-[30px] sm:text-[44px] md:text-[48px] font-extrabold leading-tight"
          ...
          <div className="mt-1">
            <div className="inline-block">
              <h3 
                className="text-[#A20000] text-[15px] sm:text-[28px] md:text-[30px] font-extrabold tracking-wide inline-flex items-center"
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
              >
                CV. YUDHA PRATAMA
                <span
          ...
          <div ref={descRef}>
            <p 
              className="mt-6 text-[10px] sm:text-[17px] md:text-[18px] leading-relaxed text-[#383737]/80"
          ...
          <button className="mt-6 px-6 py-2 sm:px-8 sm:py-3 border border-[#A20000] rounded-full text-[#A20000] text-[11px] sm:text-base font-semibold tracking-wide hover:bg-[#A20000] hover:text-white transition-all duration-300 ease-out">
            BACA LEBIH LANJUT
          </button>
        </div>
      </div>
